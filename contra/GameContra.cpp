...
int GameContra::initGame(HINSTANCE hIstance)
{
	...
	{
		MessageBox(m_register->getHandle(), L"Can't Create Windows", L"Error", MB_OK);
		return 0;
	}
	// graphics
	...
	{
		MessageBox(m_register->getHandle(), L"Can't Create Graphic Device", L"Error", MB_OK);
		return 0;
	}
	...
	{
		MessageBox(m_register->getHandle(), L"Can't Create input device", L"Error", MB_OK);
		return 0;
	}
	if (!m_input->initKeyboard(m_register->getHandle()))
	{
		MessageBox(m_register->getHandle(), L"Can't Create Keyboard", L"Error", MB_OK);
		return 0;
	}
	...
	{
		MessageBox(m_register->getHandle(), L"Can't Create Sprite ", L"Error", MB_OK);
		return 0;
	}


	// text
	m_text = new CText(m_pFont, const_cast < char*>("Arial"), m_graphic->getPd3Device(), m_spriteHandler);

	if (!m_text->init())
	{
		MessageBox(m_register->getHandle(), L"Can't Create Text", L"Error", MB_OK);
		return 0;
	}
	...
	if (m_audio == NULL)
	{
		MessageBox(m_register->getHandle(), L"Can't Init Sound ", L"Error", MB_OK);
		return 0;
	}

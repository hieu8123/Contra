...
public:
	...
	// Lấy instance của CResourcesManager
	static CResourcesManager* GetInstance();

	// Chuẩn bị CResourcesManager để sử dụng trong game
	...
	// ----------------Method-------------- - /
		...
		// Tải một texture từ ResourceID
		void loadTexture(ResourceID id);

	// Lấy texture từ ResourceID
	CTexture* getTexture(ResourceID id);

	// Tải tất cả texture object trong map
	void loadAllInMap();

	// Hủy tất cả texture object trong map
	void unloadAllInMap();

	// Tải danh sách các object không nằm trong QuadTree từ file text
	vector<CObject*> loadFileTextOBNoneTree(string path);

	// Tải danh sách các tile từ file text
	vector<int> loadFileTextBG(string path);

	// Tải danh sách node từ file text
	vector<QTNode*> loadFileTextNode(string path);

	// Tải danh sách object từ file text
	vector<CObject*> loadFileTextOB(string path);

	// Tách chuỗi để thêm vào CObject
	CObject* splitTextPushOB(string str);

	// Tách chuỗi để thêm vào Node
	QTNode* splitTextPushNode(string str);

	// Đọc file confi_default

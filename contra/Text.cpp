...
bool CText::init()
{
	...
	//mbstowcs_s(m_fontDesc.FaceName, m_fontName, strlen(m_fontName) + 1); // name will be something like "Arial"
...
void CText::draw(string str)
{
	...
	m_font->DrawText(m_sprite, // <-- the sprite
		ConvertStringToLPCWSTR(str),  // <-- the text
...
void CText::draw()
{
	...
	m_font->DrawText(m_sprite, // <-- the sprite
		ConvertStringToLPCWSTR(a),  // <-- the text
		-1, // <-- num characters in string  -1 means its null terminated
		&rect, // <--- position <limits>
		DT_TOP | DT_LEFT, // <--- alignment in the rect
		D3DCOLOR_XRGB(0, 0, 0)); // Color   

...
ScenePlay::ScenePlay()
{
	...
	else
	{
		...
		{
			_weapon_player1->SetWeaponType(WPS);
		}
	}
...
void ScenePlay::init()
{
	...
	m_spDoor->setPostion(D3DXVECTOR2(190, 90));
...
void ScenePlay::update(float time)
{
	...
	if (m_state == PS_InGame)
	{
		...
		{
			m_state = PS_NextMap;
			CResourcesManager::GetInstance()->m_nextMap++;
			m_timeDuring = 60;
		}
	}
	...
	if (_enemies.size() > 0)
	{
		for (vector<CObject*>::iterator i = _enemies.begin(); i != _enemies.end(); i++)
		{
			...
			if (enemy->_hp > 0)
			{
				...
				{
					if (enemy->getSpecificType() == Boss1)
					{
		...
		for (vector<CObject*>::iterator i = _items.begin(); i != _items.end(); i++)
		{
			...
			if (item->_hp == 0 && item->_enable)
			{
				if (_player1->_physical.Collision(&item->_physical) != NoCollision)
				{
					...
					{
					case ItemM_Stand:
						_weapon_player1->SetWeaponType(WPM);
						break;
					case ItemF_Stand:
						_weapon_player1->SetWeaponType(WPF);
						break;
					case ItemS_Stand:
						_weapon_player1->SetWeaponType(WPS);
						break;
					case ItemL_Stand:
						_weapon_player1->SetWeaponType(WPL);
						break;
					case ItemB_Stand:
						_player1->Undying();
						break;
